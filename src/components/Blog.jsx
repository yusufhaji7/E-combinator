...
const Blog = () => {
  ...
  return (
    <div className="blog-container flex flex-wrap space-x-4 space-y-8">
      ...
      {blogPosts.map((post) => (
        <Link key={post.id} to={`/blog/${post.id}`} className="block w-full sm:w-1/2 md:w-1/3 lg:w-1/2 xl:w-1/4 p-0 mb-4 hover:shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
